...
import rtaclient.sjmanager.tableDAGNode;
...
//           SUPPRIMER LA MAP avec toutes les cardinalités dans les tableNodes
...
//            sql = "Select a from ((select b from #postal_code) join (select d from e) on c.m = e.m) join (Select f from g) on c.m = g.m;";
            ...
            //SELECT u.id, u.name, u.postal_code, us.prefecture_name, us.city_name, us.address FROM users u, user_stocks us WHERE u.postal_code = us.code;

            //SELECT u.id, u.name, SUM (us.number * s.ending_price) FROM users u, user_stocks us, #stocks s WHERE u.id = us.user_id AND us.code = s.code AND u.id = 3 AND s.market = '東証2部' GROUP BY u.id, u.name;

            //SELECT v.value FROM #test1 u, #test2 v WHERE u.id_a=v.id_a AND u.id_b=v.id_b;

            // SELECT s.point, s.observatory_number FROM #snow s, #observatory o WHERE s.observatory_number=o.number AND s.observatory_number>13000;

            // SELECT a.author, b.url FROM #bib_paper_soshin a, #bib_paper_soshin_2 b WHERE a.author=b.author AND a.award_year='1992';

            //SELECT s.number, i.title FROM #bib_magazine_soshin s, #bib_magazine_internet i WHERE i.publisher = s.publisher;

            //select p.observatory_number, s.year from #precipitation p, #snow s where p.observatory_number=s.observatory_number;

            // SELECT p.payment_id, r.inventory_id FROM #payment p, #rental r WHERE p.rental_id=r.rental_id AND p.amount>0;

            //select p.payment_id, r.customer_id, c.email, s.username, i.inventory_id, f.description from #inventory i, #customer c, #payment p, #rental r, #staff s, #film f where c.customer_id=p.customer_id and p.rental_id = r.rental_id and p.staff_id=s.staff_id and r.staff_id = s.staff_id and s.staff_id=i.store_id and i.film_id=f.film_id and f.length>100 ;
            ...
            List<TableConnector> localConnector = parser.getLocalConnector();

            for (TableConnector tc : remoteConnector) {
                if (tc.getAccessMethod().equals("API")){
                    String table_name = InitialExecution.executeAPIinit(tc,parser);
                    tc.setTbName(table_name);
                    System.out.println("exectName"+table_name); // A SUPPRIMER
                } else if (tc.getAccessMethod().equals("direct")){
                    try{
                        InitialExecution.getDirectStat(tc,parser);
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                }
            ...
            for (TableConnector tc : localConnector){
                try{
                    InitialExecution.getLocalStat(tc,parser);
                } catch (SQLException e) {
                    e.printStackTrace();
                }
            }

           /* System.out.println("DEBUT DES TABLES ARBRES");
            for (Map.Entry current : parser.getTableDAGRoots().entrySet()){
                tableDAGRoot ar =(tableDAGRoot) current.getValue();
                ar.printTree();
                System.out.println("\n");
            }
            ...
            /*for (TableConnector tc : remoteConnector) {
                if (tc.getAccessMethod().equals("direct")){
                    System.out.println("lalala");
                    try {
                        System.out.println("CE QUE JE VEUX LA "+ QueryExecutor.getDirectStat(tc,parser));
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                }
            ...
            /*System.out.println("Debut des benefices");
            for (SemiJoinOperation sj : parser.getSJPlan()) {
                sj.print();
                System.out.println("benef"+sj.estimateBenefitsSJ(parser));
            }
            System.out.println("FIN des benefices");*/

            SemiJoinOperation.executeAllSJ(parser);

           /* System.out.println("DEBUT DES Attribut ARBRES");
            for (TableConnector tc : remoteConnector){
                System.out.println(tc.getTbName());
                for (Map.Entry current : tc.getAttributDAGRoots().entrySet()){
                    attributDAGRoot ar =(attributDAGRoot) current.getValue();
                    ar.printTree();
            ...
            for (TableConnector tc : remoteConnector) {
                try {
                    ...
                    if (tc.getAccessMethod().equals("API")||tc.getAccessMethod().equals("on_site")) {
...
//                        String rs_json = GetData.fromSQL("SELECT s.ending_price, s.code, s.market FROM stocks s WHERE code IN ('1434-T', '2499-T', '1828-T', '2345-T', '2668-T', '6042-T', '6237-T', '6772-T', '6982-T', '7544-T', '9708-T', '9873-T', '9537-T', '8912-T', '7965-T')", tc.getAccessName());
                        ...
                        String jsonData = GetData.result(tc.getHost(),tc.getTbName());
                        QueryExecutor.insertFromJson(tc.getAccessName(), jsonData, tmpdate, false);
...
//                        con_remote = DBConnect.connectVPN();
                        ...
                        ps_remote = con_remote.prepareStatement(tc.getSQL());
                        // mock
                ...
                if (joins != null) {
                    for (Join j : joins) {
                        ...
                        if (tbName.equals(t.getName()) && tbAlias.equals(t.getAlias())) {
                            t.setName(t.getName() + "_" + tmpdate);                                         //la aussi
                            // PostgreSQLの場合schemaをset
                            if (GlobalEnv.getDriver().equals("postgresql")) {
