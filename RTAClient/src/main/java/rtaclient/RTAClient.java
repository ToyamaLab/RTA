...
//            sql = "Select a from ((select b from #postal_code) join (select d from e) on c.m = e.m) join (Select f from g) on c.m = g.m;";
            ...
            try {
                QueryExecutor.createTmp(); //TmpDBの作成
            ...
            for (TableConnector tc : remoteConnector) {
                try {
                    ...
                    switch (tc.getAccessMethod()) {
						case "direct":
							System.out.println("select Items = " + tc.getSelectItem().toString());
                     ...
                     System.out.println("tcSQL = " + tc.getSQL());
                     // mock
...
//                     ps_remote = con_remote.prepareStatement("SELECT s.ending_price, s.code, s.market FROM stocks s WHERE code IN ('1434-T', '2499-T', '1828-T', '2345-T', '2668-T', '6042-T', '6237-T', '6772-T', '6982-T', '7544-T', '9708-T', '9873-T', '9537-T', '8912-T', '7965-T')");
                     ...
                     QueryExecutor.insertFromResultSet(tc.getDBMS(), con_tmp, rs_remote, tmpdate, false /* 一時的な結果 */);
						...
						case "web":
							String rs_json = GetData.fromSQL(tc.getSQL(), tc.getAccessName());
                     // mock
...
//                     String rs_json = GetData.fromSQL("SELECT s.ending_price, s.code, s.market FROM stocks s WHERE code IN ('1434-T', '2499-T', '1828-T', '2345-T', '2668-T', '6042-T', '6237-T', '6772-T', '6982-T', '7544-T', '9708-T', '9873-T', '9537-T', '8912-T', '7965-T')", tc.getAccessName());
                     QueryExecutor.insertFromJson(tc.getDBMS(), rs_json, tmpdate, false);
                     break;
    ...
    public static void changeTableTimestamp(SelectBody sb, String tbName, Alias tbAlias) {
        switch (sb.getClass().getSimpleName()) {
            ...
            case "PlainSelect":
                ...
                if (tbName.equals(tb.getName()) &&((tbAlias == null && tb.getAlias() == null) ||tbAlias != null && tbAlias.equals(tb.getAlias()))) {
